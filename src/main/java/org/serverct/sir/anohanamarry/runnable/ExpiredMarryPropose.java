...
public class ExpiredMarryPropose extends BukkitRunnable {
    ...
    public ExpiredMarryPropose(Player sender, Player receiver) {
    ...
    public void run() {
        ...
        if(!playerData.getStatus().equals(StatusType.Married)) {
            ...
            if(queue.contains(targetPlayer.getName())) {
                ...
                Language.getInstance().sendSubtitle(targetPlayer.getName(), Language.getInstance().getMessage("Common.Propose.Marry.Result.Expired.Subtitle").replace("%receiver%", player.getName()));
                targetPlayer.sendMessage(Language.getInstance().getMessage(MessageType.INFO, "Common.Propose.Marry.Result.Expired.Message").replace("%receiver%", player.getName()));

                player.sendMessage(Language.getInstance().getMessage(MessageType.INFO, "Commands.Propose.Marry.Expired").replace("%sender%", targetPlayer.getName()));
            }
        }
    }
}
