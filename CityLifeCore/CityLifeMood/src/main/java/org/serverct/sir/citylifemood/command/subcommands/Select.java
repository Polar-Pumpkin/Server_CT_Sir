                    ...
                    if(selection != null) {
                        if(args.length >= 6) {
                            ...
                            if(CommonUtil.isInteger(args[3]) && CommonUtil.isInteger(args[4])) {
                                ...
                                playerSender.sendMessage(locale.getMessage(MessageType.WARN, "Commands", "Unknown.Number"));
                                return true;
                            }
                            reason = args[5];
                            if(args.length > 6) {
                            ...
                            playerSender.sendMessage(
                                    locale.getMessage(MessageType.INFO, "Commands", "Area.Created")
                                            .replace("%id%", args[2])
                                            .replace("%step%", args[3])
                ...
                case "rem":
                    if(args.length == 3) {
                        if(AreaManager.getInstance().getAreasMap().containsKey(args[2])) {
                            ...
                            playerSender.sendMessage(locale.getMessage(MessageType.INFO, "Commands", "Area.Deleted").replace("%id%", args[2]));
                        } else {
                            playerSender.sendMessage(locale.getMessage(MessageType.WARN, "Commands", "Area.NotExist"));
                        }
                    }
                    break;
                default:
