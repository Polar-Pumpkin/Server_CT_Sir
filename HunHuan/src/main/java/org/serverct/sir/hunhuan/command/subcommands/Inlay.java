    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if(sender instanceof Player) {
            ...
            if((targetHunguItem != null && targetHunguItem.getType() != Material.AIR) && hunhuanManager.isHungu(targetHunguItem)) {
                ...
                HunhuanData target = hunhuanManager.getHungu(targetHunguItem);
                locale.debug("获取目标魂骨数据成功.");

                if(itemInHand != null && itemInHand.getType() != Material.AIR) {

                    if(hunhuanManager.isHungu(itemInHand)) {
                        user.sendMessage(locale.getMessage(MessageType.WARN, "Command", "Invalid.Item.NoHunhuan"));
                        return true;
                    }
                    ...
                    if(itemInHand.getAmount() == 1) {
                        ...
                        if(handItemMeta.hasLore()) {
                            ...
                            if(handItemLore != null && !handItemLore.isEmpty()) {
                                ...
                                List<HunhuanData> handItemHungu = HunhuanUtil.getInstance().hasHungu(handItemLore);
