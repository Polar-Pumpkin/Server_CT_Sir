package org.serverct.sir.hunhuan.hooks;
...
public class VaultHook {
    ...
    public void loadVault() {
        if (!setupEconomy()) {
            //Hungu.getInstance().setVaultHook(false);
            Bukkit.getLogger().info("  > 未找到 Vault, 扣费模块不可用.");
        } else {
            //Hungu.getInstance().setVaultHook(true);
            Bukkit.getLogger().info("  > 已连接 Vault.");
        }
    }

    private boolean setupEconomy() {
        if(HunHuan.getInstance().getServer().getPluginManager().getPlugin("Vault") != null) {
            RegisteredServiceProvider<Economy> rsp = HunHuan.getInstance().getServer().getServicesManager().getRegistration(Economy.class);
            if (rsp == null) {
                return false;
            }
